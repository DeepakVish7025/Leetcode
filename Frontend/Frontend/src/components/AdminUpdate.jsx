...
// Zod schema matching the problem schema (identical to your create page)
...
function UpdateProblem() {
  ...
  const [isLoading, setIsLoading] = useState(true); // To show a loader while fetching data
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
    reset // Use reset to populate the form with fetched data
  } = useForm({
    resolver: zodResolver(problemSchema),
    defaultValues: { // Set default structure to avoid uncontrolled component errors
      title: '',
      description: '',
      difficulty: 'easy',
      tags: 'array',
  ...
  useEffect(() => {
    const fetchProblemData = async () => {
      try {
        // Use the correct endpoint to fetch a single problem by its ID
        const response = await axiosClient.get(`/problem/update/${id}`); 
        // Populate the form with the fetched data
        reset(response.data); 
      } catch (error) {
        alert(`Error fetching problem data: ${error.response?.data?.message || error.message}`);
        navigate('/admin/update'); // Redirect back if the problem isn't found
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Handle form submission to update the problem
  ...
  // --- Reusing styles from your AdminPanel component ---
  ...
  return (
    <div className="min-h-screen bg-gray-900 py-8 px-4">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {/* The entire form structure is identical to AdminPanel */}
