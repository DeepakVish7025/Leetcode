...
// import axiosClient from '../utils/axiosClient'; // Assuming this and Redux setup are external
...
//       // AIzaSyAWIeZWueZ8gC_hlk0q6X6bJJDgr_3W6Ls
...
import React, { useState, useEffect } from 'react';
import { useParams, Link } from 'react-router-dom'; // Changed to react-router-dom for Link
import { MOCK_COURSES, MOCK_TESTIMONIALS, APP_ROUTES, courses } from '../utils/constants';
import TestimonialCard from '../components/coursePage/TestimonialCard';
import { StarIcon, ClockIcon, UsersIcon, CheckCircleIcon, BookOpenIcon, ChevronDownIcon, ChevronUpIcon, VideoCameraIcon, AcademicCapIcon, ArrowRightIcon } from '../components/Icons/CoursesPageIcons';
import axiosClient from '../utils/axiosClient'; // Assuming this and Redux setup are external
...
const CourseOverviewPage = () => {
  ...
  useEffect(() => {
    ...
    // Mock testimonials for now, if needed
  ...
  if (!course) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-black via-gray-900 to-black text-gray-200 p-4">
        <h1 className="text-3xl font-bold mb-4">Course not found.</h1>
        <Link to={APP_ROUTES.courses} className="px-6 py-3 bg-orange-600 hover:bg-orange-700 text-white font-semibold rounded-lg transition-colors duration-200 shadow-md">
          Back to Courses
        </Link>
      </div>
    );
  ...
  return (
    <div className="bg-gradient-to-br from-black via-gray-900 to-black text-gray-100 min-h-screen">
      {/* Hero Section */}
      <div className="bg-gray-900/60 py-16 px-4 sm:px-6 lg:px-8 border-b border-gray-800">
        <div className="max-w-7xl mx-auto lg:grid lg:grid-cols-3 lg:gap-x-12">
          <div className="lg:col-span-2">
            <p className="text-base font-semibold text-orange-400 uppercase tracking-wide">{course.category.join(', ')}</p>
            <h1 className="mt-2 text-4xl font-extrabold text-white sm:text-5xl">{course.title}</h1>
            <p className="mt-4 text-xl text-gray-300">{course.tagline}</p>
            <p className="mt-3 text-sm text-gray-500">Last updated: {course.publishedAt}</p>
            <div className="mt-6 flex items-center space-x-4">
              <div className="flex items-center">
                <img src={course.instructorProfileImageUrl} alt={course.instructorName} className="w-10 h-10 rounded-full mr-2 border-2 border-orange-500" />
                <span className="text-sm text-gray-300">Created by <span className="font-semibold text-white">{course.istructorName}</span></span>
              </div>
              {course.rating && (
            ...
            <div className="mt-4 flex items-center space-x-6 text-sm text-gray-500">
              <div className="flex items-center"><ClockIcon className="w-5 h-5 mr-1.5 text-orange-400" /> {course.itemCount}</div>
              <div className="flex items-center"><AcademicCapIcon className="w-5 h-5 mr-1.5 text-orange-400" /> {course.difficulty}</div>
              {course.enrollmentCount && <div className="flex items-center"><UsersIcon className="w-5 h-5 mr-1.5 text-orange-400" /> {course.enrollmentCount?.toLocaleString()} students</div>}
            </div>
          </div>
          <div className="mt-10 lg:mt-0 lg:col-span-1 relative group rounded-2xl transition-all duration-300">
            <div className="absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-orange-600 group-hover:animate-pulseBorder pointer-events-none" />
            <div className="relative bg-gray-900/80 rounded-2xl shadow-xl overflow-hidden">
              <img src={course.url} alt={course.title} className="w-full h-56 object-cover" />
              <div className="p-6">
                {course.isFree ? (
                  <p className="text-3xl font-bold text-green-500">FREE</p>
                ) : (
                  <p className="text-3xl font-bold text-white">${course.price.toFixed(2)}</p>
                )}
                <Link
                  to={firstLectureUrl}
                  className="mt-6 w-full flex items-center justify-center py-3 px-6 bg-orange-600 hover:bg-orange-700 text-white font-semibold rounded-lg transition-colors duration-200 shadow-md group"
                >
                  <span>{course.isFree ? 'Start Learning Now' : 'Enroll in Course'}</span>
                  <ArrowRightIcon className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" />
                </Link>
                <p className="mt-3 text-xs text-gray-500 text-center">30-Day Money-Back Guarantee (for paid courses)</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="lg:grid lg:grid-cols-3 lg:gap-x-12">
          <div className="lg:col-span-2">
            {/* Curriculum */}
            <section className="mb-12">
              <h2 className="text-3xl font-bold text-white mb-6 border-b border-gray-800 pb-4">Course Curriculum</h2>
              <div className="space-y-3">
                {course.curriculum.map((module) => (
                  <div key={module.id} className="relative group rounded-lg transition-all duration-300">
                    <div className="absolute inset-0 rounded-lg border-2 border-transparent group-hover:border-orange-600 group-hover:animate-pulseBorder pointer-events-none" />
                    <div className="relative bg-gray-900/80 rounded-lg shadow-md overflow-hidden">
                      <button
                        onClick={() => toggleModule(module.id)}
                        className="w-full flex justify-between items-center p-4 bg-gray-800/40 hover:bg-gray-700/40 transition-colors focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-inset border-b border-gray-700"
                        aria-expanded={expandedModules[module.id]}
                        aria-controls={`module-content-${module.id}`}
                      >
                        <h3 className="text-lg font-medium text-white">{module.title}</h3>
                        <ChevronDownIcon className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${expandedModules[module.id] ? 'transform rotate-180' : ''}`} />
                      </button>
                      <div
                        id={`module-content-${module.id}`}
                        className={`overflow-hidden transition-all duration-500 ease-in-out ${expandedModules[module.id] ? 'max-h-screen' : 'max-h-0'}`}
                      >
                        <ul className="divide-y divide-gray-800">
                          {module.lectures?.map((lecture) => (
                            <li key={lecture.contentDetails?.videoId}>
                              <Link to={APP_ROUTES.courseContent.replace(':courseId', course.id).replace(':lectureId', lecture.contentDetails?.videoId)} className="flex items-center justify-between p-4 hover:bg-gray-800/60 group transition-colors">
                                <div className="flex items-center">
                                  <VideoCameraIcon className="w-5 h-5 text-orange-500 mr-3 flex-shrink-0 group-hover:text-orange-400" />
                                  <span className="text-sm text-gray-200 group-hover:text-white">{lecture.snippet?.title}</span>
                                </div>
                                <span className="text-xs text-gray-500 group-hover:text-gray-400">{lecture.duration}</span>
                              </Link>
                            </li>
                          ))}
                        </ul>
            ...
            {/* Description */}
            <section className="mb-12 relative group rounded-2xl transition-all duration-300">
              <div className="absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-orange-600 group-hover:animate-pulseBorder pointer-events-none" />
              <div className="relative bg-gray-900/80 p-6 rounded-2xl shadow-md">
                <h2 className="text-3xl font-bold text-white mb-4 border-b border-gray-800 pb-4">Description</h2>
                <div className="prose prose-sm dark:prose-invert text-gray-300 max-w-none" dangerouslySetInnerHTML={{ __html: course.description.replace(/\n/g, '<br/>') }}></div>
              </div>
            </section>

            {/* Testimonials */}
            {testimonials.length > 0 && (
              <section className="relative group rounded-2xl transition-all duration-300">
                <div className="absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-orange-600 group-hover:animate-pulseBorder pointer-events-none" />
                <div className="relative bg-gray-900/80 p-6 rounded-2xl shadow-md">
                  <h2 className="text-3xl font-bold text-white mb-6 border-b border-gray-800 pb-4">Student Feedback</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {testimonials.map(testimonial => (
                      <TestimonialCard key={testimonial.id} testimonial={testimonial} />
                    ))}
          ...
          {/* Sidebar (Instructor Info) */}
          <aside className="mt-12 lg:mt-0 lg:col-span-1 relative group rounded-2xl transition-all duration-300">
            <div className="absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-orange-600 group-hover:animate-pulseBorder pointer-events-none" />
            <div className="relative bg-gray-900/80 p-6 rounded-2xl shadow-md sticky top-24">
              <h3 className="text-2xl font-bold text-white mb-4 border-b border-gray-800 pb-4">About the Instructor</h3>
              <div className="flex items-center mb-3">
                <img src={course.instructorProfileImageUrl} alt={course.istructorName} className="w-16 h-16 rounded-full mr-4 border-2 border-orange-500" />
                <div>
                  <h4 className="text-lg font-medium text-white">{course.istructorName}</h4>
                  <p className="text-sm text-orange-400">Lead Instructor</p>
                </div>
              </div>
      ...
      <style>{`
        @keyframes pulseBorder {
          0% { border-width: 3px; border-color: rgba(249, 115, 22, 0.4); } /* Orange-500 */
          50% { border-width: 3px; border-color: rgba(249, 115, 22, 1); }
          100% { border-width: 3px; border-color: rgba(249, 115, 22, 0.4); }
        }
        .animate-pulseBorder {
